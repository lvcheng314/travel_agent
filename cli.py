...
class AligoCLI:
    ...
    async def initialize_system(self):
        ...
        with self.console.status("初始化中...", spinner="dots"):
            ...
            # LangGraph workflow: explicit intention -> orchestration state flow.
            from langgraph_workflow import TravelWorkflow
            self.workflow = TravelWorkflow(self.intention_agent, self.orchestrator)
        ...
        try:
            async def run_workflow():
                if self.workflow is not None:
                    state = await self.workflow.ainvoke_from_intention(intention_result)
                    if state.get("error"):
                        raise ValueError(state["error"])
                    return state["result"]
                return await self.orchestrator.reply(intention_result)
